...
#pragma mark ZtxFxAudioPlayer Class


@implementation ZtxFxAudioPlayer
...
-(void)processAudioThread:(id)param
{
	...
	@autoreleasepool {
		...
		}
		{
			...
			mTotalFramesInFile = [mReader fileNumFrames] / kOversample + ZtxFxLatencyFrames(kOversample*mSampleRate);
#ifdef DEBUG
			...
			mZtx = ZtxFxCreate(kZtxQualityBest, kOversample*mSampleRate, mNumChannels);
			if (!mZtx) {
				printf("!! ERROR !!\n\n\tCould not create ZtxFx instance\n\tCheck sample rate!\n");
				exit(-1);
			}	
			...
			// Allocate buffer for ZTX output
			short **audioIn = AllocateAudioBufferSInt16(mNumChannels, numFrames);
			short **audioOut = AllocateAudioBufferSInt16(mNumChannels, ZtxFxMaxOutputBufferFramesRequired(2.0, 1.0, numFrames));
		...
		again:
			...
			for(;;) {
				...
				framesOut = ZtxFxProcess(mTimeFactor, mPitchFactor, audioIn, audioOut, nf, mZtx);
			...
			if (framesOut == kZtxErrorDemoTimeoutReached) {
