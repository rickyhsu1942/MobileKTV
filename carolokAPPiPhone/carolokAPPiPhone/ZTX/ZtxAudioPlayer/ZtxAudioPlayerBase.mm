...
{    
	...
	ZtxAudioPlayerBase *Self = (__bridge ZtxAudioPlayerBase *)inRefCon;
	if (!Self) return -1;
...
#pragma mark ZtxAudioPlayerBase Class


@implementation ZtxAudioPlayerBase
...
{
	if (mDelegate) {
		if ([mDelegate respondsToSelector:@selector(ztxPlayerDidFinishPlaying:successfully:)] == YES) {
			[mDelegate ztxPlayerDidFinishPlaying:player successfully:flag];
		} else {
			NSLog(@"\t!!! PROBLEM: %@ is a delegate of class %@ and does not respond to ztxPlayerDidFinishPlaying:", [[mDelegate class] description], [[self class] description]);
		}
	}
}
...
-(void)setupInstanceWithUrl:(NSURL*)inUrl numChannels:(int)channels
{
	...
	mNumChannels = ZtxValidateNumChannels(channels);
	...
	[mySession setPreferredSampleRate:audioFormat.mSampleRate error:nil];
	// make sure we got what we wanted
	audioFormat.mSampleRate = [mySession sampleRate];
	*/
	// Initialize unit
...
-(BOOL)prepareToPlay
{
	if (!mIsPrepared) {
		...
		mLoopCount = 0;
		mIsProcessing = YES;
		// this kicks off our background worker thread that does the actual ZTX processing
...
@synthesize mAudioUnit, mReader, mVolume, mNumberOfLoops, mNumChannels, mAudioBufferReadPos, mAudioBuffer, mZtx, mLoopCount, mPeak, mAudioBufferWritePos, mIsProcessing, mIsPrepared, mTotalFramesGenerated, mTotalFramesPlayed, mFramePositionInInputFile, mLastResetPositionInFile, mTotalFramesInFile, mTotalFramesConsumed;
